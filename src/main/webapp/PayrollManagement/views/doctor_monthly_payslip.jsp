...
<p><a href="view_monthly_payslip.jsp" style="text-decoration:none;"><h2>Monthly Payrolls</h2></a></p>
	...
	<tbody>
		...
		<% for(Doctor doctor : DoctorRelative.findAllDoctors()){ %>
			<tr>
				...
				<td>
					<form action="<%=request.getContextPath()%>/DoctorUpdate" method="post">
						<input type="hidden" name="id" value="<%= doctor.getId() %>">
						<input type="submit" value="Update" class="btn btn-primary btn-change7">
					</form>
				<td>
					<form action="<%=request.getContextPath()%>/DoctorDelete" method="post">
						<input type="hidden" name="id" value="<%= doctor.getId() %>">
