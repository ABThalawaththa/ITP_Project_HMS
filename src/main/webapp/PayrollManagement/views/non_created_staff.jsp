...
<%@page import="prm.model.TestStaff"%>
<%@page import="prm.service.StaffRelative"%>
...
<p><a href="<%=request.getContextPath()%>/PayrollManagement/views/create_payslip.jsp" style="text-decoration:none;"><h2>Create Payroll</h2></a></p>
<br>
		...
		<%for(TestStaff staff : StaffRelative.findAllStaff()){ %>
			<tr>
				...
				<%if(!StaffRelative.checkStaff(staff.getId())){ %>
					<form action="<%=request.getContextPath()%>/CreatePaySheet" method="post">
				...
				<%}else{ %>
					<form action="<%=request.getContextPath()%>/ViewPaySheet" method="post">
						<input type="hidden" name="id" value="<%= staff.getId() %>">
						<input type="hidden" name="name" value="<%= staff.getName() %>">
						<input type="hidden" name="designation" value="<%= staff.getDesignation() %>">
						<input type="hidden" name="basicSalary" value="<%= String.format("%.2f",staff.getBasicSalary()) %>">
